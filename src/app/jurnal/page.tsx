...
export default function JurnalPublic() {
  // Tempat penampungan data sementara di memori website
  const [daftarJurnal, setDaftarJurnal] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  // FUNGSI 1: Mengambil data dari server secara otomatis saat halaman dibuka
  useEffect(() => {
    const ambilData = async () => {
      try {
        // Membuat perintah: "Ambil data dari jurnal_kkn, urutkan dari tanggal terbaru"
        const q = query(collection(db, "jurnal_kkn"), orderBy("tanggal", "desc"));
        
        // Eksekusi perintah pengambilan data
        const querySnapshot = await getDocs(q);
        
        const dataSementara: any[] = [];
        querySnapshot.forEach((doc) => {
          // Gabungkan ID dokumen bawaan server dengan isi teksnya
          dataSementara.push({ id: doc.id, ...doc.data() });
        });
        
        // Simpan data yang sudah dirapikan ke penampungan
        setDaftarJurnal(dataSementara);
      } catch (error) {
        console.error("Gagal mengambil data:", error);
      } finally {
        // Matikan animasi loading setelah selesai mengambil data (sukses/gagal)
        setLoading(false);
      }
    };
  ...
  // FUNGSI 2: Merapikan format tulisan tanggal komputer menjadi format kalender Indonesia
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-10 max-w-4xl">
        ...
        {/* Kondisi Tampilan */}
        {loading ? (
          // 1. Tampilan saat web masih memanggil server Google
          <div className="text-center text-gray-500 font-semibold mt-20 animate-pulse">
            Memuat data jurnal...
          </div>
        ) : daftarJurnal.length === 0 ? (
          // 2. Tampilan jika berhasil konek server, tapi laci jurnal masih kosong
          ...
          // 3. Tampilan jika data berhasil ditemukan (Proses Looping)
          <div className="space-y-6">
            ...
            >
                ...
                {/* TAMBAHAN BARU: Memunculkan gambar jika data gambar tersedia di laci database */}
                {jurnal.gambar && (
                <img 
                    src={jurnal.gambar} 
                    alt={jurnal.judul} 
